...
// incluiBoleto=true para eventos (cliente paga em parcelas ao longo dos meses)
const criarLinkMercadoPago=async(titulo,valor,referencia,incluiBoleto=false)=>{
  try{
    const paymentMethods=incluiBoleto
      ?{installments:12}// boleto + PIX + cartão
      :{excluded_payment_types:[{id:'ticket'}],installments:12};// só PIX + cartão
      ...
      <div>
        ...
        <div style={{background:"#fff",border:"1.5px solid #e8e0d8",borderRadius:12,padding:14,marginBottom:12}}>
          ...
          {Number(agendamento.valor||0)>0&&(()=>{
            ...
            const isEvento=svcConfig?.grupo==="evento";
            const valor=Number(agendamento.valor||0);
            const entrada=Math.round(valor*0.3*100)/100;
            const subtitulo=isEvento?"PIX + Cartão + Boleto (evento)":"PIX + Cartão";
            const gerarLink=async(tipo)=>{
              setGerandoLink(true);
              const v=tipo==="entrada"?entrada:valor;
              const ref=tipo==="entrada"?`ENTRADA-${agendamento.id}`:`TOTAL-${agendamento.id}`;
              const titulo=tipo==="entrada"
                ?`Entrada 30% — ${agendamento.servico}`
                :`${agendamento.servico}${agendamento.modalidade?" — "+agendamento.modalidade:""} — Total`;
              const link=await criarLinkMercadoPago(titulo,v,ref,isEvento);
            ...
            return(
              <div style={{marginTop:14,paddingTop:14,borderTop:"1px solid #f0e8e0"}}>
                <p style={{fontSize:11,color:"#aaa",margin:"0 0 10px",fontWeight:600}}>🔗 Gerar link de cobrança — Mercado Pago ({subtitulo})</p>
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
                  <button disabled={gerandoLink} onClick={()=>gerarLink("entrada")} style={{padding:"12px 8px",borderRadius:10,background:gerandoLink?"#f0ece8":"#f5f0eb",border:"1.5px solid #e8e0d8",cursor:gerandoLink?"default":"pointer",textAlign:"center"}}>
                    <p style={{fontSize:11,fontWeight:700,color:"#b8967e",margin:"0 0 3px"}}>Entrada 30%</p>
                    <p style={{fontSize:16,fontWeight:700,color:"#1a1a1a",margin:0,fontFamily:"'Cormorant Garamond',serif"}}>R$ {entrada.toFixed(2).replace(".",",")}</p>
                  </button>
                  <button disabled={gerandoLink} onClick={()=>gerarLink("total")} style={{padding:"12px 8px",borderRadius:10,background:gerandoLink?"#f0ece8":"#e8f5e8",border:"1.5px solid #c8e6c9",cursor:gerandoLink?"default":"pointer",textAlign:"center"}}>
                    <p style={{fontSize:11,fontWeight:700,color:"#2e7d32",margin:"0 0 3px"}}>Pagamento total</p>
                    <p style={{fontSize:16,fontWeight:700,color:"#1a1a1a",margin:0,fontFamily:"'Cormorant Garamond',serif"}}>R$ {valor.toFixed(2).replace(".",",")}</p>
                  </button>
                </div>
    ...
    return(
      <div>
        ...
        {(()=>{
          ...
          // Campos ordenados para típico
          const CAMPOS_TIP = [
            ['esquecer_mundo','O que faz esquecer o mundo por 10 minutos?'],
            ['personagem','Personagem ou música favorita?'],
            ['mania_roupa','Mania com roupas?'],
            ['barulhos','Como lida com barulhos inesperados?'],
            ['reacao_querer','Reação quando quer muito algo?'],
            ['toque','Gosta de abraço?'],
            ['tempo_amb_tip','Precisa de tempo para se adaptar?'],
            ['ensaio_tip','Já fez ensaios antes?'],
            ['desc_ensaio_tip','Como foi o ensaio?'],
            ['autoriza_imagem','Autoriza uso das fotos no portfólio?'],
            ['extra_tip','Informações extras'],
          ];
          // Campos ordenados para atípico
          const CAMPOS_ATIP = [
            ['transtorno','Qual transtorno?'],
            ['tempo_transtorno','Há quanto tempo descobriu?'],
            ['fazTerapia','Faz terapia?'],
            ['desc_terapias','Quais terapias?'],
            ['medicamentos','Faz uso de medicamentos?'],
            ['estereotipias','Tem estereotipias?'],
            ['prevenir_crises','Como prevenir crises?'],
            ['contornar','Como contornar momentos difíceis?'],
            ['incomoda','O que incomoda?'],
            ['incomoda_outro','O que incomoda (outro)?'],
            ['gosta','Do que gosta?'],
            ['brinquedos','Brinquedos favoritos?'],
            ['brinquedos_outro','Brinquedo favorito (outro)?'],
            ['social_atip','Como é a interação social?'],
            ['comunicacao','Como se comunica?'],
            ['comunicacao_outro','Comunicação (outro)?'],
            ['tempo_amb_atip','Precisa de tempo para se adaptar?'],
            ['ensaio_atip','Já fez ensaios antes?'],
            ['desc_ensaio_atip','Como foi o ensaio?'],
            ['outros_eventos','Já foi a outros eventos com fotógrafo?'],
            ['postar_transtorno','Autoriza postar sobre o transtorno?'],
            ['extra_atip','Informações extras'],
          ];
          ...
          return(
            <div style={{marginBottom:12}}>
              ...
              {listaFilhos.map((f,i)=>{
                ...
                const isAtip = anData.atipico==="Sim";
                const campos = isAtip ? CAMPOS_ATIP : CAMPOS_TIP;
                const algumPreenchido = campos.some(([k])=>anData[k]&&anData[k]!=="");
                return(
                  <div key={i} style={{marginBottom:6}}>
                    <div onClick={()=>setFilhoSelecionado(isOpen?null:i)}
                      ...
                      <div>
                        ...
                        <p style={{margin:"2px 0 0",fontSize:11,color:"#888"}}>{isAtip?"🧡 Atípico":"🌿 Típico"}</p>
                      </div>
                      <span style={{fontSize:12,color:"#b8967e",fontWeight:600}}>{isOpen?"▲ fechar":"📋 ver anamnese"}</span>
                    </div>
                    {isOpen&&(
                      <div style={{background:"#fff",border:"1.5px solid #D9A7B4",borderTop:"none",borderRadius:"0 0 10px 10px",padding:"14px 14px 6px"}}>
                        {!algumPreenchido&&(
                          <p style={{fontSize:12,color:"#bbb",textAlign:"center",padding:"8px 0 4px",margin:0}}>Anamnese não preenchida para este filho.</p>
                        )}
                        {campos.map(([k,label])=>{
                          const v = anData[k];
                          const vazia = !v||v==="";
                          // Oculta "Como foi o ensaio?" se não fez ensaio
                          if(k==='desc_ensaio_tip'&&anData['ensaio_tip']!=="Sim") return null;
                          if(k==='desc_ensaio_atip'&&anData['ensaio_atip']!=="Sim") return null;
                          return(
                            <div key={k} style={{marginBottom:10,paddingBottom:10,borderBottom:"1px solid #f5eef0"}}>
                              <span style={{fontSize:10,color:"#b8967e",display:"block",fontWeight:700,marginBottom:2,textTransform:"uppercase",letterSpacing:".05em"}}>{label}</span>
                              <span style={{fontSize:13,color:vazia?"#ccc":"#333",lineHeight:1.6,fontStyle:vazia?"italic":"normal"}}>
                                {vazia?"Não respondido":Array.isArray(v)?v.join(", "):String(v)}
                              </span>
                            </div>
                          );
                        })}
      ...
      {auth.email===PHOTOGRAPHER.email&&(
        <>
          <div style={{display:"flex",gap:6,marginBottom:20}}>
            {[["agenda","📅 Agenda"],["crm","🗂 CRM"],["disponibilidade","🗓 Horários"],["kanban","📋 Kanban"]].map(([t,l])=><button key={t} onClick={()=>setTab(t)} style={{flex:1,padding:"10px 4px",borderRadius:8,fontSize:11,fontWeight:600,background:tab===t?"#1a1a1a":"#fff",color:tab===t?"#fff":"#666",border:"2px solid "+(tab===t?"#1a1a1a":"#e8e0d8"),cursor:"pointer"}}>{l}</button>)}
          </div>
