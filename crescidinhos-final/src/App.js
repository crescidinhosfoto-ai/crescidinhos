    ...
    <div>
      ...
      {step===3&&!semAgenda&&(
        <div>
          ...
          <CalendarComDisponibilidade selectedDate={date} onSelectDate={async(d)=>{setDate(d);setTime(null);setHorariosDisp([]);setLoadingHorarios(true);let slots=await fetchHorariosDisponiveis(d);if(!slots||slots.length===0){try{const r=await sb(`disponibilidades?data=eq.${d}&select=horarios&limit=1`);if(r?.length)slots=r[0].horarios||[];}catch{}}setHorariosDisp(slots||[]);setLoadingHorarios(false);}}/>
          {date&&(
            <div style={{marginTop:14}}>
              <p style={{fontSize:12,color:"#999",marginBottom:10}}>Horários disponíveis em <strong>{formatDateBR(date)}</strong>:</p>
              {loadingHorarios&&<p style={{fontSize:12,color:"#b8967e"}}>Verificando horários...</p>}
              {!loadingHorarios&&horariosDisp.length===0&&<p style={{fontSize:12,color:"#bbb"}}>Nenhum horário disponível nesta data.</p>}
              <div style={{display:"flex",flexWrap:"wrap",gap:8}}>
                {horariosDisp.map(t=><button key={t} onClick={()=>setTime(t)} style={{padding:"8px 14px",borderRadius:8,fontSize:13,border:time===t?"2px solid #1a1a1a":"2px solid #e8e0d8",background:time===t?"#1a1a1a":"#fff",color:time===t?"#fff":"#1a1a1a",cursor:"pointer"}}>{t}</button>)}
              </div>
            </div>
          )}
...
// ─── CALENDAR COM DISPONIBILIDADES (substitui Calendar para clientes) ─────────
function CalendarComDisponibilidade({ selectedDate, onSelectDate }) {
  const today = new Date();
  const [vy,setVy] = useState(today.getFullYear());
  const [vm,setVm] = useState(today.getMonth());
  ...
  return (
    <div>
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:12}}>
        <button onClick={()=>vm===0?(setVm(11),setVy(y=>y-1)):setVm(m=>m-1)} style={{background:"none",border:"none",fontSize:20,cursor:"pointer",color:"#1a1a1a",padding:"4px 10px"}}>‹</button>
        <span style={{fontFamily:"'Cormorant Garamond',serif",fontSize:16,fontWeight:600}}>{["Jan","Fev","Mar","Abr","Mai","Jun","Jul","Ago","Set","Out","Nov","Dez"][vm]} {vy}</span>
        <button onClick={()=>vm===11?(setVm(0),setVy(y=>y+1)):setVm(m=>m+1)} style={{background:"none",border:"none",fontSize:20,cursor:"pointer",color:"#1a1a1a",padding:"4px 10px"}}>›</button>
      </div>
      <div style={{display:"grid",gridTemplateColumns:"repeat(7,1fr)",gap:2,marginBottom:4}}>
        {["Dom","Seg","Ter","Qua","Qui","Sex","Sáb"].map(w=><div key={w} style={{textAlign:"center",fontSize:10,color:"#aaa",fontWeight:600,padding:"3px 0"}}>{w}</div>)}
        {cells.map((d,i)=>{
          if(!d) return <div key={i}/>;
      ...
      {auth.email===PHOTOGRAPHER.email&&(
        <>
          <div style={{display:"flex",gap:6,marginBottom:20}}>
            {[["agenda","📅 Agenda"],["crm","🗂 CRM"],["disponibilidade","🗓 Disponibilidade"]].map(([t,l])=><button key={t} onClick={()=>setTab(t)} style={{flex:1,padding:"10px 4px",borderRadius:8,fontSize:12,fontWeight:600,background:tab===t?"#1a1a1a":"#fff",color:tab===t?"#fff":"#666",border:"2px solid "+(tab===t?"#1a1a1a":"#e8e0d8"),cursor:"pointer"}}>{l}</button>)}
          </div>
